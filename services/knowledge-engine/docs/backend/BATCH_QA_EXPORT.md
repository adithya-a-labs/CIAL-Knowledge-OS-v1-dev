...
on-premises Docker service from the repository root with
`scripts\start_qdrant.bat`, verify it with the authenticated
`scripts/check_qdrant_health.py` helper, and construct `Phase4Config` with
`qdrant_mode="server"` and `qdrant_url="http://localhost:6335"`. This does not
