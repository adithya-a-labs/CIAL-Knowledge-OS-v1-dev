...
from backend.app.schemas.notes import LinkDocumentCreate, NoteCreate, NoteList, NoteRecord, NoteUpdate, TagCreate
...
def create_note(payload:NoteCreate,request:Request,svc:NoteService=Depends(service)): return call(request,svc,"create",payload.title)
...
def update_note(note_id:uuid.UUID,payload:NoteUpdate,request:Request,svc:NoteService=Depends(service)): return call(request,svc,"update",note_id,payload)
...
def restore_note(note_id:uuid.UUID,request:Request,svc:NoteService=Depends(service)): return call(request,svc,"restore",note_id)
@router.post("/workspaces/me/notes/{note_id}/duplicate",response_model=NoteRecord,status_code=201)
def duplicate_note(note_id:uuid.UUID,request:Request,svc:NoteService=Depends(service)): return call(request,svc,"duplicate",note_id)
