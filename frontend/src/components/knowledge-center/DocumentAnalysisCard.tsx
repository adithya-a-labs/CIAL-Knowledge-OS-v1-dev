...
export function AnalysisVersionBadge({summary}:{summary:SummaryRecord}){
  return <div className="flex flex-wrap items-center gap-1.5 text-[11px] text-slate-500"><span className="rounded-full bg-slate-100 px-2 py-1">{summary.summary_length}</span>{summary.document_version_number?<span>Version {summary.document_version_number}</span>:null}{summary.completed_at?<span className="inline-flex items-center gap-1"><Clock3 size={11}/>{new Date(summary.completed_at).toLocaleString()}</span>:null}{summary.stale||summary.status==='stale'?<span className="rounded-full bg-amber-100 px-2 py-1 font-medium text-amber-800">Stale</span>:null}</div>;
}
...
export function AnalysisProgress({progress}:{progress?:AnalysisProgressValue}){
  ...
  return <section className="rounded-xl border border-primary/20 bg-primary/5 p-4" aria-live="polite" data-testid="analysis-progress"><div className="flex items-center gap-2 text-sm font-semibold text-slate-900"><LoaderCircle size={16} className="text-primary motion-safe:animate-spin"/>Preparing document analysis…</div><p className="mt-2 text-xs text-slate-600">{detail||'Loading the authorized document version'}</p><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-slate-200" role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={percent}><div className="h-full rounded-full bg-primary transition-[width] motion-reduce:transition-none" style={{width:`${percent}%`}}/></div></section>;
}

function ItemList({items}:{items:GroundedAnalysisItem[]}){return <ul className="mt-2 space-y-2">{items.map((item,index)=><li key={`${item.text}-${index}`} className="text-xs leading-5 text-slate-700"><span>{item.text}</span> <span className="whitespace-nowrap font-medium text-primary">{item.citation_ids.map((id)=>`[${id}]`).join(' ')}</span></li>)}</ul>}
...
export function AnalysisSections({payload}:{payload:DocumentAnalysisPayload}){
  ...
  return <div className="space-y-3" data-testid="analysis-sections">{payload.sections.filter((section)=>section.items.length).map((section)=><section key={section.heading} className="rounded-xl border border-slate-200 p-3"><h3 className="text-sm font-semibold text-slate-900">{section.heading}</h3><ItemList items={section.items}/></section>)}{collections.filter(([,items])=>items.length).map(([heading,items])=><section key={heading} className="rounded-xl border border-slate-200 p-3"><h3 className="text-sm font-semibold text-slate-900">{heading}</h3><ItemList items={items}/></section>)}{payload.coverage_gaps.length?<section className="rounded-xl border border-amber-200 bg-amber-50 p-3"><h3 className="text-sm font-semibold text-amber-950">Coverage Gaps</h3><ul className="mt-2 list-disc space-y-1 pl-4 text-xs leading-5 text-amber-900">{payload.coverage_gaps.map((gap)=><li key={gap}>{gap}</li>)}</ul></section>:null}</div>;
}
...
export function AnalysisCitationList({summary,onCitation}:{summary:SummaryRecord;onCitation:(page:number,chunkId?:string|null)=>void}){
  ...
  return <section className="rounded-xl border border-slate-200 p-3" data-testid="analysis-citations"><div className="flex items-center justify-between"><h3 className="text-sm font-semibold">Citations</h3><span className="text-xs text-slate-500">{summary.citations.length}</span></div><div className="mt-2 space-y-2">{summary.citations.map((citation)=><button key={citation.citation_id} onClick={()=>onCitation(citation.page_number??1,citation.chunk_id)} className="w-full rounded-lg bg-slate-50 p-2 text-left text-xs hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><span className="font-semibold text-primary">[{citation.citation_id}]</span><span className="ml-2 text-slate-500">{citation.page_number?`Page ${citation.page_number}`:'Page not provided'}{citation.section?` · ${citation.section}`:''}</span><p className="mt-1 line-clamp-3 leading-5 text-slate-600">{citation.excerpt||'Source excerpt unavailable.'}</p></button>)}</div></section>;
}
...
export function GenerateAnalysisDialog({open,type,length,busy,onClose,onGenerate}:{open:boolean;type:DocumentAnalysisType;length:DocumentAnalysisLength;busy:boolean;onClose:()=>void;onGenerate:(type:DocumentAnalysisType,length:DocumentAnalysisLength)=>void}){
  ...
  if(!open)return null;
  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/35 p-4" role="presentation" onMouseDown={(event)=>{if(event.target===event.currentTarget&&!busy)onClose()}}><div role="dialog" aria-modal="true" aria-labelledby="generate-analysis-title" className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-2xl"><div className="flex items-start justify-between"><div><h2 id="generate-analysis-title" className="font-semibold text-slate-950">Generate document analysis</h2><p className="mt-1 text-xs leading-5 text-slate-500">Every length processes the complete current document version.</p></div><button ref={closeRef} onClick={onClose} disabled={busy} className="ce-icon-button" aria-label="Close analysis settings"><X size={16}/></button></div><label className="mt-4 block text-xs font-medium text-slate-700">Type<select value={nextType} onChange={(event)=>setNextType(event.target.value as DocumentAnalysisType)} className="mt-1 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm"><option value="overview">Overview</option><option value="detailed">Detailed</option><option value="key_points">Key points</option><option value="action_items">Action items</option></select></label><label className="mt-3 block text-xs font-medium text-slate-700">Length<select value={nextLength} onChange={(event)=>setNextLength(event.target.value as DocumentAnalysisLength)} className="mt-1 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm"><option value="brief">Brief</option><option value="standard">Standard</option><option value="detailed">Detailed</option></select></label><button onClick={()=>onGenerate(nextType,nextLength)} disabled={busy} className="ce-action ce-action-primary mt-5 h-10 w-full justify-center disabled:opacity-50">{busy?<LoaderCircle size={15} className="motion-safe:animate-spin"/>:<Sparkles size={15}/>}Generate</button></div></div>;
}
...
export default function DocumentAnalysisCard({document,onAsk,onCitation,onSuggestedQuestion}:{document:CorpusDocument;onAsk:(question?:string)=>Promise<void>;onCitation:(page:number,chunkId?:string|null)=>void;onSuggestedQuestion:(question:string)=>void}){
  ...
  if(analysis.isLoading)return <div className="rounded-xl border border-slate-200 p-4 text-sm text-slate-500">Loading document analysis…</div>;
  return <div className="space-y-3" data-testid="document-analysis-card">
    {!shown&&!analysis.isError?<section className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-5 text-center"><FileSearch className="mx-auto text-slate-400" size={28}/><h2 className="mt-3 text-sm font-semibold text-slate-950">Document analysis</h2><p className="mt-2 text-xs leading-5 text-slate-600">Generate a grounded summary, key findings, and citations for this document.</p><button onClick={()=>setDialog(true)} disabled={generate.isPending} className="ce-action ce-action-primary mt-4 h-10 px-4 disabled:opacity-50"><Sparkles size={15}/>Generate analysis</button><button onClick={()=>void onAsk()} className="ce-action mt-2 h-9 px-3 text-xs"><MessageSquareText size={14}/>Ask AI instead</button></section>:null}
    {analysis.isError?<section className="rounded-xl border border-red-200 bg-red-50 p-4 text-center"><AlertTriangle className="mx-auto text-red-600" size={24}/><h3 className="mt-2 text-sm font-semibold text-red-950">Analysis unavailable</h3><p className="mt-1 text-xs text-red-800">The document analysis endpoint could not be loaded.</p><button onClick={()=>void analysis.refetch()} className="ce-action mt-3 h-9 px-3"><RefreshCw size={14}/>Retry</button></section>:null}
    {active?<><AnalysisProgress progress={current?.progress}/><p className="px-1 text-[11px] text-slate-500">You can leave; analysis continues in background.</p></>:null}
    {failed?<section className="rounded-xl border border-red-200 bg-red-50 p-4"><h3 className="text-sm font-semibold text-red-950">Analysis failed</h3><p className="mt-1 text-xs text-red-800">{current?.error_message||'The local analysis could not be completed.'}</p>{current.retryable!==false?<button onClick={()=>doGenerate(type,length,true)} disabled={generate.isPending} className="ce-action mt-3 h-9 px-3"><RefreshCw size={14}/>Retry</button>:null}</section>:null}
    {ready&&shown?<><section className={cn('rounded-xl border p-3',shown.stale||shown.status==='stale'?'border-amber-200 bg-amber-50':'border-slate-200 bg-white')}><div className="flex items-start justify-between gap-2"><div><h2 className="text-sm font-semibold text-slate-950">{shown.title}</h2><AnalysisVersionBadge summary={shown}/></div><div className="relative"><button onClick={()=>setMenu((value)=>!value)} className="ce-icon-button" aria-label="Analysis actions" aria-expanded={menu}><ChevronDown size={15}/></button>{menu?<div className="absolute right-0 top-10 z-10 w-44 rounded-lg border border-slate-200 bg-white p-1 text-xs shadow-lg"><button onClick={()=>{setMenu(false);doGenerate(type,length,true)}} className="w-full rounded-md px-3 py-2 text-left hover:bg-slate-50">Regenerate</button><button onClick={()=>{setMenu(false);setDialog(true)}} className="w-full rounded-md px-3 py-2 text-left hover:bg-slate-50">Change type or length</button></div>:null}</div></div>{shown.stale||shown.status==='stale'?<div className="mt-3 text-xs text-amber-900"><p>This analysis was generated for an earlier document version.</p><button onClick={()=>doGenerate(type,length,false)} className="ce-action mt-2 h-8 px-2">Generate updated analysis</button></div>:null}</section><AnalysisSections payload={shown.structured_payload!}/><AnalysisCitationList summary={shown} onCitation={onCitation}/>{shown.suggested_questions?.length?<section className="rounded-xl border border-slate-200 p-3"><h3 className="text-sm font-semibold">Suggested Follow-up Questions</h3><div className="mt-2 flex flex-wrap gap-2">{shown.suggested_questions.map((question)=><button key={question} onClick={()=>onSuggestedQuestion(question)} className="rounded-lg border border-slate-200 px-2 py-1.5 text-left text-xs hover:bg-slate-50">{question}</button>)}</div></section>:null}<div className="grid grid-cols-1 gap-2 sm:grid-cols-2"><button onClick={()=>follow.mutate(shown.id)} disabled={follow.isPending||shown.status==='stale'} className="ce-action ce-action-primary h-10 justify-center disabled:opacity-50"><MessageSquareText size={15}/>Ask about this summary</button><button onClick={()=>save.mutate(shown.id)} disabled={save.isPending} className="ce-action h-10 justify-center disabled:opacity-50"><BookmarkPlus size={15}/>{save.isSuccess?'Saved':'Save to Knowledge'}</button></div></>:null}
    {generate.isError?<p role="alert" className="rounded-lg bg-red-50 p-3 text-xs text-red-800">Analysis generation could not be queued. Try again.</p>:null}
