...
function TreeNode({node,selectedDocumentId}:{node:CorpusTreeNode;selectedDocumentId:string}){
  ...
  return <div><button onClick={()=>setOpen((value)=>!value)} className="flex h-8 w-full items-center gap-1.5 rounded-md px-2 text-left text-xs font-medium text-foreground transition-colors duration-[var(--motion-duration-short)] ease-[var(--motion-ease-move)] hover:bg-muted" style={{paddingLeft:`${Math.min(node.depth,6)*12+8}px`}}><ChevronRight size={13} className={cn('motion-spatial text-muted-foreground transition-transform duration-[var(--motion-duration-short)] ease-[var(--motion-ease-move)]',open&&'rotate-90')}/><Folder size={14} className="text-primary/75"/><span className="truncate">{node.name||'Root'}</span></button>{open?<div>{node.children.map((child)=><TreeNode key={child.id??child.relative_path} node={child} selectedDocumentId={selectedDocumentId}/>)}{documents.map((document)=><button key={document.id} onClick={()=>navigate(`/knowledge/document/${document.id}`)} className={cn('flex h-8 w-full items-center gap-2 rounded-md pr-2 text-left text-xs transition-colors duration-[var(--motion-duration-short)] ease-[var(--motion-ease-move)] hover:bg-muted',document.id===selectedDocumentId&&'bg-accent font-semibold text-primary shadow-[inset_2px_0_0_hsl(var(--primary))]')} style={{paddingLeft:`${Math.min(node.depth+1,7)*12+22}px`}}><FileText size={13}/><span className="truncate">{document.name}</span></button>)}</div>:null}</div>;
}
