...
export default function StorageBreakdownChart({ data }: StorageBreakdownChartProps) {
  return (
    <div className="responsive-card border border-border bg-card p-4 shadow-sm" data-testid="storage-breakdown-chart">
      <h3 className="text-sm font-semibold text-foreground mb-3">Storage Breakdown</h3>
          ...
          {data.map((item) => (
            <div key={item.name} className="flex items-center justify-between gap-2">
              <div className="min-w-0 flex items-center gap-1.5">
                ...
                <span className="truncate text-xs text-primary">{item.name}</span>
              </div>
              <span className="text-xs font-semibold text-foreground">{item.value} GB</span>
            </div>
          ))}
        </div>
      </div>
