...
export default function PersonalStorageCard() {
  ...
  return (
    <div className="responsive-card border border-border bg-card p-4 shadow-sm sm:p-5" data-testid="personal-storage-card">
      <h2 className="text-sm font-semibold text-foreground mb-4">Your Personal Storage</h2>
      ...
      <div className="flex flex-col items-center gap-5 md:flex-row md:items-center md:gap-6">
        ...
        <div className="min-w-0 flex-1 self-stretch">
          <div className="mb-1">
            <span className="text-2xl font-bold text-foreground">{s.usedGB} GB</span>
            <span className="text-sm text-muted-foreground ml-1">/ {s.totalGB} GB used</span>
          </div>
          ...
          <p className="text-xs text-muted-foreground mb-3">{s.availableGB} GB available</p>
          ...
          <ul className="space-y-1.5">
            {STORAGE_PRIVACY_BULLETS.map((bullet) => (
              <li key={bullet} className="flex items-start gap-2">
                ...
                <span className="safe-text text-xs text-primary">{bullet}</span>
              </li>
            ))}
          </ul>
