...
export default function PdfViewer({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-[1.5rem] border border-border bg-muted">
      ...
      <div ref={viewportRef} className="scrollbar-soft min-h-0 flex-1 overflow-auto p-4">
        <Document
          file={fileUrl}
          loading={<div className="rounded-2xl border border-border bg-card p-4 text-sm text-muted-foreground">Loading PDF...</div>}
          error={<div className="rounded-2xl border border-destructive/30 bg-destructive/10 p-4 text-sm text-destructive">The PDF could not be rendered.</div>}
            ...
            {Array.from({ length: numPages || 1 }, (_, index) => {
              ...
              return (
                <div
                  ...
                  className={`pdf-page-shell rounded-[1.35rem] border bg-card p-3 shadow-sm ${pageNumber === activePage ? 'border-primary/50 shadow-[0_0_0_4px_rgba(74,124,63,0.08)]' : 'border-border'}`}
                >
