...
export function AIComposerFrame({ children, className, testId = 'compact-chat-composer' }: { children: ReactNode; className?: string; testId?: string }) {
  return <div className={cn('mx-auto grid min-h-[108px] w-full max-w-6xl grid-cols-[minmax(0,1fr)_auto] grid-rows-[minmax(3rem,auto)_auto] rounded-[1.4rem] border border-[#dfe6dc] bg-white shadow-sm transition focus-within:border-[#9fbd96] focus-within:ring-4 focus-within:ring-primary/10',className)} data-testid={testId}>{children}</div>;
}
...
export default function AIComposer({ value, onChange, onSubmit, placeholder, disabled=false, toolbar, leadingAction, textareaRef, testId='shared-ai-composer' }: { value:string; onChange:(value:string)=>void; onSubmit:()=>void|Promise<void>; placeholder:string; disabled?:boolean; toolbar?:ReactNode; leadingAction?:ReactNode; textareaRef?:RefObject<HTMLTextAreaElement|null>; testId?:string }) {
  return <AIComposerFrame testId={testId}>
    ...
    <button type="button" onClick={()=>void onSubmit()} disabled={disabled||!value.trim()} className="col-start-2 row-span-2 row-start-1 mb-3 mr-3 inline-flex h-11 w-11 self-end items-center justify-center rounded-full bg-primary text-primary-foreground shadow-sm transition hover:bg-[#285f22] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring disabled:bg-[#d9dfd7] disabled:text-slate-500 disabled:shadow-none sm:h-12 sm:w-12" aria-label="Send message" data-testid="button-send"><Send size={18}/></button>
  </AIComposerFrame>;
}
