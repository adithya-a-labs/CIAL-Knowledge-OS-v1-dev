  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-3" data-testid="context-manager-dialog">
      <div className="flex h-[min(92dvh,56rem)] w-full max-w-6xl flex-col overflow-hidden rounded-xl border border-border bg-white p-4 shadow-2xl">
        <div className="mb-3 flex gap-1 border-b border-slate-200"><button onClick={()=>setTab('knowledge')} className={`px-3 py-2 text-sm ${tab==='knowledge'?'border-b-2 border-primary font-semibold text-primary':'text-slate-500'}`}>Knowledge</button><button onClick={()=>setTab('notes')} className={`px-3 py-2 text-sm ${tab==='notes'?'border-b-2 border-primary font-semibold text-primary':'text-slate-500'}`}>Private Notes</button></div>
        {tab==='knowledge'?<CorpusExplorer
          ...
          onCancel={onClose}
        />:<div className="flex min-h-0 flex-1 flex-col"><label className="relative mb-3 block"><Search size={15} className="absolute left-3 top-2.5 text-slate-400"/><input value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="Search private notes" className="h-9 w-full rounded-lg border border-slate-200 pl-9 pr-3 text-sm"/></label><div className="min-h-0 flex-1 overflow-y-auto">{notes.isLoading?<p className="p-4 text-sm text-slate-500">Loading notes…</p>:notes.isError?<p className="p-4 text-sm text-red-700">Notes could not be loaded.</p>:notes.data?.items.map((note)=>{const selected=draftItems.some((item)=>item.type==='note'&&item.id===note.id);return <button key={note.id} onClick={()=>setDraftItems((current)=>selected?current.filter((item)=>!(item.type==='note'&&item.id===note.id)):[...current,{id:note.id,type:'note',title:note.title,relative_path:`notes/${note.id}`,updated_at:note.updated_at,is_pinned:note.is_pinned}])} className={`flex w-full items-center gap-3 border-b border-slate-100 px-3 py-3 text-left hover:bg-slate-50 ${selected?'bg-[#edf5e9]':''}`}><NotebookPen size={16} className="text-primary"/><span className="min-w-0 flex-1"><strong className="block truncate text-sm">{note.title}</strong><span className="mt-1 flex items-center gap-2 text-xs text-slate-500"><Lock size={11}/>Private · {new Date(note.updated_at).toLocaleString()}</span></span>{note.is_pinned?<Pin size={14} className="text-primary"/>:null}<span className={`h-4 w-4 rounded border ${selected?'border-primary bg-primary':''}`}/></button>})}</div><div className="mt-3 flex justify-end gap-2"><button onClick={onClose} className="rounded-lg border px-4 py-2 text-sm">Cancel</button><button onClick={()=>{onApply(draftItems);onClose();}} className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-white">Use selected context</button></div></div>}
      </div>
    </div>
  );
}
