    ...
    <div className="mx-auto flex w-full max-w-[86rem] flex-col gap-6" data-testid="departments-page">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold text-foreground">Organizational knowledge spaces</h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
            Browse department-owned documents, folders, activity, and AI context scopes.
          </p>
        </div>
        <label className="flex h-11 min-w-0 items-center gap-2 rounded-lg border border-border bg-card px-3 text-sm text-muted-foreground shadow-sm lg:w-80">
          <Search size={16} />
          <input className="min-w-0 flex-1 bg-transparent text-foreground placeholder:text-muted-foreground" placeholder="Search departments" type="search" />
        </label>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <main className="space-y-6">
          <section className="rounded-xl border border-border bg-card shadow-sm">
            <div className="border-b border-border px-4 py-3">
              <h2 className="text-sm font-semibold text-foreground">Department directory</h2>
            </div>
            <div className="divide-y divide-border">
              {DEPARTMENTS.map((dept) => {
                const IconComp = ICON_MAP[dept.icon] || Building2;
                return (
                  <article key={dept.id} className="grid gap-3 px-4 py-4 transition hover:bg-muted lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
                    <div className="flex min-w-0 items-start gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-xs text-muted-foreground">Head: {dept.headName}</p>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <section className="rounded-xl border border-border bg-card p-4 shadow-sm">
              <h2 className="text-sm font-semibold text-foreground">Department documents</h2>
            ...
            <section className="rounded-xl border border-border bg-card p-4 shadow-sm">
              <h2 className="text-sm font-semibold text-foreground">Department folders</h2>
              <div className="mt-3 divide-y divide-border">
        ...
        <aside className="space-y-4">
          <section className="rounded-xl border border-border bg-card p-4 shadow-sm">
            ...
            <div className="mt-3 space-y-3">
              {departmentActivity.map((activity) => (
                <div key={activity} className="rounded-lg border border-border bg-muted p-3 text-xs leading-5 text-foreground">
                  {activity}
                </div>
              ))}
            </div>
          ...
          <section className="rounded-xl border border-border bg-card p-4 shadow-sm">
            <h2 className="text-sm font-semibold text-foreground">AI scope hook</h2>
            <p className="mt-2 text-sm leading-6 text-muted-foreground">Start from a department to keep answers grounded to that organizational space.</p>
          ...
          <section className="rounded-xl border border-border bg-card p-4 shadow-sm">
            <h2 className="text-sm font-semibold text-foreground">Permissions hooks</h2>
            <p className="mt-2 text-xs leading-5 text-muted-foreground">Department-level access and ownership metadata can plug into this surface without changing the browsing model.</p>
          </section>
        </aside>
      </div>
    </div>
