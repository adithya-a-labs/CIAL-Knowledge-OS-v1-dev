    ...
    <div className="mx-auto flex w-full max-w-[96rem] flex-col gap-5" data-testid="workspace-page">
      ...
      {treeQuery.isLoading || preferencesQuery.isLoading ? <div className="grid gap-3"><Skeleton className="h-12" /><Skeleton className="h-96" /></div> : activeTab === 'notes' ? <NotesWorkspace createSignal={createNoteSignal} initialNoteId={new URLSearchParams(location.split('?')[1] ?? '').get('note')} /> : activeTab === 'saved' ? <SavedKnowledgeWorkspace/> : activeTab === 'files' || activeTab === 'overview' ? (
        <>
          ...
          <div className="grid gap-5 lg:grid-cols-2"><section className="rounded-xl border border-slate-200 bg-white p-4"><div className="flex items-center justify-between"><h2 className="flex items-center gap-2 text-sm font-semibold"><NotebookPen size={16} className="text-primary" />My Notes</h2><button onClick={() => setActiveTab('notes')} className="text-xs font-semibold text-primary">Open notes</button></div><p className="mt-3 text-xs text-slate-500">Private notes are stored in your workspace and available from the Notes tab.</p></section><section className="rounded-xl border border-slate-200 bg-white p-4"><div className="mb-3 flex items-center justify-between"><h2 className="flex items-center gap-2 text-sm font-semibold"><Bot size={16} className="text-primary" />Recent AI Conversations</h2><Link href={ASSISTANT_FRESH_PATH} className="text-xs font-semibold text-primary">Open AI</Link></div>{(summary?.recent_conversations ?? []).slice(0, preferences.recentItemLimit).map((conversation) => <Link href={assistantConversationPath(conversation.id)} key={conversation.id} className="flex items-center gap-2 rounded-lg px-2 py-2 text-sm hover:bg-slate-50"><ChevronsUpDown size={14} className="text-primary" /><span className="truncate">{conversation.title}</span></Link>)}</section></div>
        </>
