    ...
    <div className="fluid-section" data-testid="knowledge-graph-page">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Visualize relationships between knowledge entities across CIAL.</p>
        </div>
      </div>

      <div className="flex flex-col gap-4 xl:flex-row">
        {/* Graph Panel */}
        <div className="responsive-card min-w-0 flex-1 overflow-hidden border border-border bg-card shadow-sm">
          {/* Toolbar */}
          <div className="flex flex-col gap-3 border-b border-border px-4 py-3 sm:flex-row sm:items-center">
            <div className="relative min-w-0 flex-1 sm:max-w-xs">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-1.5 text-sm bg-muted border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-ring/30"
                data-testid="graph-search"
              />
              {search && (
                <button onClick={() => setSearch('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-[#7a9a72] hover:text-foreground">
                  <X size={12} />
                </button>
              )}
            </div>
            <span className="text-xs text-muted-foreground">{KG_NODES.length} nodes · {KG_EDGES.length} connections</span>
          </div>
        ...
        <div className="grid gap-4 xl:w-72 xl:flex-shrink-0">
          {selectedNode ? (
            <div className="responsive-card border border-border bg-card p-5 shadow-sm" data-testid="graph-detail-panel">
              <div className="flex items-start justify-between mb-3">
                ...
                <button onClick={() => setSelectedNode(null)} className="p-1 rounded hover:bg-accent text-muted-foreground"><X size={14} /></button>
              </div>
              ...
              <p className="text-xs text-muted-foreground mt-3 leading-relaxed">{selectedNode.description}</p>
              {selectedNode.count !== undefined && (
                <div className="mt-3 p-3 rounded-lg bg-muted border border-border">
                  <p className="text-2xl font-bold text-primary">{selectedNode.count.toLocaleString()}</p>
                  <p className="text-xs text-muted-foreground">Connected items</p>
                </div>
              )}
              <div className="mt-3 pt-3 border-t border-border">
                <p className="text-xs font-semibold text-foreground mb-2">Connected to</p>
                <div className="space-y-1">
                  {KG_EDGES.filter(e => e.source === selectedNode.id || e.target === selectedNode.id).slice(0, 5).map(edge => {
                    ...
                    return (
                      <button key={edge.id} onClick={() => setSelectedNode(other)} className="flex items-center gap-2 w-full text-left hover:bg-accent rounded-lg px-2 py-1.5 transition-colors">
            ...
            <div className="responsive-card border border-border bg-card p-5 text-center shadow-sm" data-testid="graph-hint-panel">
              <div className="w-12 h-12 rounded-full bg-accent flex items-center justify-center mx-auto mb-3">
                <Info size={20} className="text-primary" />
              </div>
              <p className="text-sm font-medium text-foreground">Click any node</p>
              <p className="text-xs text-muted-foreground mt-1">Select a node to see its details, connections, and related knowledge entities.</p>
              <div className="mt-4 space-y-2 text-left">
                ...
                <div className="space-y-1">
                  {KG_NODES.filter(n => n.type !== 'hub').slice(0, 5).map(n => (
                    <button key={n.id} onClick={() => setSelectedNode(n)} className="flex items-center gap-2 w-full hover:bg-accent rounded-lg px-2 py-1.5 transition-colors text-left">
