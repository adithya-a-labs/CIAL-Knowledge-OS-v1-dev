...
export default function NotebooksPage() {
  ...
  return <div className="mx-auto flex w-full max-w-[86rem] flex-col gap-6" data-testid="notebook-library">
    ...
    <div className="flex flex-wrap items-center gap-3"><label className="relative min-w-[16rem] max-w-xl flex-1"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground"/><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search notebooks" className="pl-9" /></label><span className="rounded-full border border-border bg-card px-3 py-1.5 text-xs text-muted-foreground">Personal workspace</span></div>
